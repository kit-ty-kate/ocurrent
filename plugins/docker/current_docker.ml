...
module Raw = struct
  ...
  let build ~docker_context ?enable_submodules ?schedule ?timeout ?(squash=false) ?dockerfile ?pool ?(build_args=[]) ~pull commit =
    let dockerfile =
...
module Make (Host : S.HOST) = struct
  ...
  let build ?enable_submodules ?schedule ?timeout ?squash ?label ?dockerfile ?pool ?build_args ~pull src =
    ...
    Raw.build ~docker_context ?enable_submodules ?schedule ?timeout ?squash ?dockerfile ?pool ?build_args ~pull commit
